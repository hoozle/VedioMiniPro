...
public class RegisterLoginController extends BaseController{
    ...
    public JSONResult login(@RequestBody User user) throws Exception {
        ...
        if(null != user){
            user.setPassword("");
            UserVO userVO = addRedisSessionTokenToUser(user);
    ...
    private UserVO addRedisSessionTokenToUser(User user) {
        String uniqueToken = UUIDUtil.getUUID();

        redisUtil.set(Constant.USER_REDIS_SESSION + ":" + user.getId(), uniqueToken, 1000 * 60 * 30);
